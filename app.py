...
# Configure test files directory
TEST_FILES_DIR = os.path.join(os.path.dirname(__file__), 'samples')

def get_available_test_files():
    """Get list of available test files from samples directory"""
    if not os.path.exists(TEST_FILES_DIR):
        return []
    
    test_files = []
    for file in Path(TEST_FILES_DIR).glob('*.test'):
        test_files.append({
            'name': file.name,
            'path': str(file),
...
<body>
    <div class="container">
        ...
        <div class="content">
            <div class="upload-section">
                <div class="tabs">
                    <button class="tab active" onclick="switchTab('existing')">Select Test File</button>
                    <button class="tab" onclick="switchTab('upload')">Upload New File</button>
                </div>
                
                <form id="testForm">
                    <!-- Tab 1: Select from existing files -->
                    <div id="existing-tab" class="tab-content active">
                        <div class="form-group">
                            <label>Available Test Files (from samples/)</label>
                    ...
                    <div id="upload-tab" class="tab-content">
                        <div class="form-group">
                            <label for="testfile">Upload Test File (.test)</label>
                            <input type="file" id="testfile" name="testfile" accept=".test">
                        </div>
                    </div>
    ...
    <script>
        ...
        // Load available test files on page load
        ...
        async function loadTestFiles() {
            try {
                ...
                const listContainer = document.getElementById('testFilesList');
                
                if (files.length === 0) {
                    listContainer.innerHTML = '<div style="padding: 20px; text-align: center; color: #999;">No test files found in samples/ directory</div>';
                    return;
                }
                
                listContainer.innerHTML = files.map(file => `
                    <div class="test-file-item" onclick="selectTestFile('${file.name}')">
                        <div class="test-file-name">${file.name}</div>
                        <div class="test-file-size">${formatBytes(file.size)}</div>
                    </div>
                `).join('');
            } catch (error) {
        ...
        function selectTestFile(filename) {
            selectedTestFile = filename;
            
            // Update UI
            document.querySelectorAll('.test-file-item').forEach(item => {
                item.classList.remove('selected');
            });
            event.target.closest('.test-file-item').classList.add('selected');
        }
        ...
        document.getElementById('testForm').addEventListener('submit', async (e) => {
            ...
            if (activeTab === 'existing-tab') {
                ...
                endpoint = '/run-tests-existing';
                body = JSON.stringify({ filename: selectedTestFile, command: command });
            } else {
...
def list_test_files():
    ...
    files = get_available_test_files()
    return jsonify(files)

@app.route('/run-tests-existing', methods=['POST'])
def run_tests_existing():
    """Run tests from an existing file in samples directory"""
    try:
        data = request.get_json()
        filename = data.get('filename')
        command = data.get('command', 'echo {input}')
        ...
        if not os.path.abspath(file_path).startswith(os.path.abspath(TEST_FILES_DIR)):
